					...
					if (current_el.id != "speech_tooltip")
					{
						...
						if (isOnScreen(current_el) == true)
						{
							if (current_el.nodeName == "INPUT" && current_el.type.match(/^(button|radio|reset|submit)$/i))
						 		...
						 		break;
						 	else if (current_el.nodeName.match(/^(BUTTON|A|OPTION|SELECT)$/i))
						 		break; // then accept this element
		...
		for (var i = 0; i < times; i++)
		{
			setTimeout(function(){ 
				...
				if (el_to_click.nodeName == "OPTION") {
					...
					if (el_to_click.parentNode.size > 1) el_to_click.parentNode.dispatchEvent(event); // Version 1.0.1 - Fire change event
					if (el_to_click.parentNode.size <= 1) el_to_click.parentNode.size = 5; // Version 1.0.1 - Open select box
					else if (el_to_click.parentNode.size == 5) el_to_click.parentNode.size = 1; // Version 1.0.1 - Close select box	
				}
				if (el_to_click.nodeName == "SELECT") {
					if (el_to_click.size <= 1) el_to_click.size = 5; // Version 1.0.1 - Open select box
					else if (el_to_click.size == 5) el_to_click.parentNode.size = 1; // Version 1.0.1 - Close select box	
				}
				/* Gmail "Compose" button only works on "mouseup" event */
				var event = document.createEvent('MouseEvents');
				event.initEvent("mousedown", true, false); // try (click|mousedown|mouseup), bubble, cancelable
